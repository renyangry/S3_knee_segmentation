...
filename = os.listdir(bone_path)[2]
...
#------------------------------------------------------------------------------------------------------------#
# step 2 - crop the distal femur - 150 mm^3 volume 
distal_femur = rec_lf_seg[:, :, 0:volume_slice]
# sitk_image = sitk.GetImageFromArray(distal_femur)
# sitk_image_int = sitk.Cast(sitk_image, sitk.sitkUInt8) 
# reference_surface = sitk.LabelContour(sitk_image_int)
# surface_array = sitk.GetArrayFromImage(reference_surface)
# nib.save(nib.Nifti1Image(distal_femur, original_affine, original_header), os.path.join(out_dir, 'distal_femur.nii.gz'))
# nib.save(nib.Nifti1Image(surface_array, original_affine, original_header), os.path.join(out_dir, 'distal_femur_surface.nii.gz'))
...
# volume_80 = np.percentile(volume, 80)
# volume_20 = np.percentile(volume, 20)
# volume_60 = np.percentile(volume, 60)
# volume_10 = np.percentile(volume, 10)
# volume_90 = np.percentile(volume, 90)
plt.plot(volume_mean, 'x', color='green', label='Mean')
# plt.plot(volume_80, 'x', color='red', label='80th Percentile')
# plt.plot(volume_20, 'x', color='yellow', label='20th Percentile')
# plt.plot(volume_60, 'x', color='blue', label='60th Percentile')
# plt.plot(volume_10, 'x', color='black', label='10th Percentile')
# plt.plot(volume_90, 'x', color='purple', label='90th Percentile')
plt.legend()
plt.show()
...
print(f'{filename},{cond},{max_diameters_list[i]},{ME_coordinates[i]},{LE_coordinates[i]},{coordinates_int[i]}\n')
...
new_data = ants.from_numpy(data,origin=rec_img.origin,spacing=rec_img.spacing,direction=rec_img.direction)
results = ants.decrop_image(new_data, img)
ants.image_write(data, os.path.join(out_dir, str('result_'+filename)))
